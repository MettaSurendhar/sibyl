...
export default function LibraryScreen({ navigation }) {
  ...
  useEffect(() => {
    navigation.setOptions({
      tabBarStyle: editMode ? { display: 'none' } : { backgroundColor: theme.surface, borderTopColor: theme.border },
    });
  ...
  const filtered = useMemo(() => {
    ...
    if (filter.tagIds.length) {
      list = list.filter((e) => e.categoryId && filter.tagIds.includes(e.categoryId));
    }
    if (filter.datePreset) {
      const range = filter.datePreset === 'custom' ? filter.customRange : dateRangeForPreset(filter.datePreset);
      if (range) {
  ...
  const sections = useMemo(() => groupByDate(filtered, !editMode), [filtered, editMode]);
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.bg }]}>
          ...
          {editMode ? (
            ...
            <>
              <TouchableOpacity onPress={() => setSearchOpen((v) => !v)} style={styles.headerBtn}>
                <Feather name="search" size={20} color={searchOpen ? theme.accent : theme.text} />
              </TouchableOpacity>
              <TouchableOpacity onPress={() => setFilterOpen(true)} style={styles.headerBtn}>
                <Feather name="filter" size={20} color={activeFilterCount ? theme.accent : theme.text} />
                {activeFilterCount > 0 && (
              ...
              <TouchableOpacity onPress={() => setMenuOpen((v) => !v)} style={styles.headerBtn}>
      ...
      {menuOpen && !editMode && (
        <>
          ...
          <View style={[styles.dropdown, { top: insets.top + 46, backgroundColor: theme.surface, borderColor: theme.border }]}>
            ...
            >
              <Feather name="edit-3" size={17} color={theme.text} style={{ marginRight: 10 }} />
            ...
            >
              <Feather name="settings" size={17} color={theme.text} style={{ marginRight: 10 }} />
      ...
      <SectionList
        ...
        contentContainerStyle={{ padding: 16, paddingBottom: editMode ? 120 : 16 }}
        ...
        renderItem={({ item }) => (
          <EntryRow
            ...
            onPressOpen={(e) => navigation.navigate('Playback', { entryId: e.id })}
            onSeek={seekEntry}
      ...
      {editMode && (
        ...
        >
          <TouchableOpacity disabled={!selectedIds.length} onPress={handleShare} style={styles.actionBtn}>
            <Feather name="share-2" size={20} color={selectedIds.length ? theme.text : theme.textMuted} />
            <Text style={{ color: selectedIds.length ? theme.text : theme.textMuted, fontSize: 12, marginTop: 4 }}>Share</Text>
          </TouchableOpacity>
          <TouchableOpacity disabled={selectedIds.length !== 1} onPress={handleRename} style={styles.actionBtn}>
            <Feather name="edit-2" size={20} color={selectedIds.length === 1 ? theme.text : theme.textMuted} />
            <Text style={{ color: selectedIds.length === 1 ? theme.text : theme.textMuted, fontSize: 12, marginTop: 4 }}>Rename</Text>
          </TouchableOpacity>
          <TouchableOpacity disabled={selectedIds.length !== 1} onPress={handleSetRingtone} style={styles.actionBtn}>
            <Feather name="bell" size={20} color={selectedIds.length === 1 ? theme.text : theme.textMuted} />
            <Text style={{ color: selectedIds.length === 1 ? theme.text : theme.textMuted, fontSize: 12, marginTop: 4 }}>Ringtone</Text>
          </TouchableOpacity>
          <TouchableOpacity disabled={!selectedIds.length} onPress={handleDelete} style={styles.actionBtn}>
            <Feather name="trash-2" size={20} color={selectedIds.length ? '#E5605A' : theme.textMuted} />
            <Text style={{ color: selectedIds.length ? '#E5605A' : theme.textMuted, fontSize: 12, marginTop: 4 }}>Delete</Text>
          </TouchableOpacity>
        </View>
      )}
...
const styles = StyleSheet.create({
  ...
  sectionLabel: { fontSize: 12, fontWeight: '700', marginBottom: 8, marginTop: 12, textTransform: 'uppercase', letterSpacing: 0.5 },
  actionBar: {
    position: 'absolute',
    bottom: 0,
    left: 0,
