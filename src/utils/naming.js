...
export function renderTemplate(template, { tag, count, timestamp = Date.now() } = {}) {
  let result = template || '{tag} <count>';
  result = result.replace(/{tag}/g, tag || 'Recording');
  result = result.replace(/<count>/g, String(count ?? 1));
  result = result.replace(/<date:([^>]+)>/g, (_, pattern) => formatDateWithPattern(timestamp, pattern));
  result = result.replace(/<time:([^>]+)>/g, (_, pattern) => formatTimeWithPattern(timestamp, pattern));
