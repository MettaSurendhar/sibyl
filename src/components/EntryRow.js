    ...
    <TouchableOpacity
      ...
      style={[
        styles.row,
        {
          ...
          borderColor: isActiveHere ? entry.categoryColor || theme.accent : theme.border,
        },
      ]}
    >
      <View style={styles.topLine}>
        <View style={{ flex: 1 }}>
          ...
          <Text style={[styles.subtitle, { color: theme.textMuted }]}>
            {entry.categoryName || 'Untagged'} · {showAbsoluteDate ? fullDateTimeLabel(entry.updatedAt) : timeLabel(entry.updatedAt)}
          </Text>
        </View>
        ...
        {editMode ? (
          <TouchableOpacity onPress={() => onToggleSelect(entry.id)} style={styles.checkboxWrap}>
            <Feather
...
const styles = StyleSheet.create({
  ...
  miniTimeRow: { flexDirection: 'row', justifyContent: 'space-between', marginTop: -6 },
